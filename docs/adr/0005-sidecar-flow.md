...
- **Code:** [`sidecar/`](../../sidecar), [`deploy/docker-compose/envoy-stack/`](../../deploy/docker-compose/envoy-stack)
...
`../../../sidecar`, and brings up six containers. No control-plane
database, no API calls: the sidecar reads one YAML file.

```bash
./run.sh --rebuild   # after changing the sidecar library
...
tier 2 alone: the `sidecar` library running as a process.
...
| `mask.enabled` | replace | The mirror of `policy.enforce`: a protocol whose frames cannot be rewritten says false against a global true. |
| `mask.rules` | replace, when the listener's list is non-empty | A rule owns an entity type. Concatenating two lists leaves two rules competing for one entity. A listener setting only `enabled` keeps the global rules. |

Why each field merges the way it does, and what a lane must carry before a
`defer` or a gate is allowed to start, is [ADR-0006](0006-sidecar-config-defaults-and-overrides.md).

`daemon.buildLanes` resolves the merge once at startup and reports every broken
...
```
                  inspect.Register(factory)   from codec init()
                        ↓
 inspect.New(Postgres) ─┴─> Inspector{codec, buf, maxBuffer 8 MiB}
                                     │
                                     ↓ codec.Decode
   ┌─── codec/postgres ────────────────────────────────┐
...
`sidecar/lexer/conformance/` is a separate Go module that parses every
...
where the identity supplies them (`sidecar/session/session.go:174-206`). The
...
```
sidecar            (0 deps) ──declares interfaces──┐
config/yaml        (1 dep)  gopkg.in/yaml.v3       │
pii/alcatraz       (2 deps) ───────────────────────┘ implements both
cmd                         ──> daemon.Main(version, det, yaml.Load)
```
...
`sidecar` in their own process pays for libhoop and nothing beyond it.
...
(`sidecar/proxy/proxy.go:86-93`). A listener exposes it as
...
- **No SSH lane.** sidecar ships no SSH codec. The argument that lane made
