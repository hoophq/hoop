...
var catalog = map[string]Flag{
	...
	"beta.mssql_native_guardrails": {
		...
		Description: "Enforce guardrails on native MSSQL (TDS) protocol sessions: SQLBatch and RPC sp_executesql/sp_prepare/sp_prepexec statements are reconstructed (across TDS packet boundaries) and validated against input rules before reaching the server; a match blocks the query and returns a TDS error to the client. Fail-closed by design: statements performing dynamic string execution (EXEC(...)/sp_executesql), and RPC parameters of unsupported types (TVP, XML, UDT, TEXT/NTEXT/IMAGE, SQL_VARIANT) whose content cannot be inspected, are refused rather than forwarded. Stored-procedure bodies are not inspected (only the invocation). Output (result-set) rules are not yet enforced natively; a connection carrying them is refused. Enforcement lives in the agent, so the gateway only admits a guarded native MSSQL session to an agent that advertises the capability — an older agent is refused, never run unguarded. When off, the gateway refuses guarded native MSSQL sessions as before.",
		Default:     true,
		Stability:   StabilityBeta,
