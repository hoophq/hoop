...
// RiskLevel represents the assessed risk of a session command, query, or request.
...
// Result holds the outcome of a single AI classification.
type Result struct {
	RiskLevel   RiskLevel
	Title       string
	Explanation string
...
// SystemPrompt is the default system prompt driving the risk classification.
// It is exported so the gateway can surface it as a configurable default in the
// admin UI without duplicating the text.
const SystemPrompt = `You are a security-focused execution risk classifier for commands, scripts, and database queries.
...
var riskToolSchema = ToolInputSchema{
...
// Analyze sends the given command/query/request to the AI provider behind the
// supplied client and returns a risk classification with a short title and
// explanation.
//
...
//
// customPrompt, when non-empty, is prepended to the default system prompt so
// the tool-calling contract (LowRisk/MediumRisk/HighRisk) is preserved.
func Analyze(ctx context.Context, client LLMClient, content, customPrompt string) (*Result, error) {
	if client == nil {
