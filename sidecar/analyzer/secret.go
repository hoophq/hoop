...
var ErrCredentialPerms = errors.New("sidecar/analyzer: credential file is readable by group or other")
...
func ReadSecretFile(path string) (Secret, error) {
	if path == "" {
		return Secret{}, errors.New("sidecar/analyzer: empty credential path")
	}
	info, err := os.Stat(path)
	if err != nil {
		return Secret{}, fmt.Errorf("sidecar/analyzer: credential file: %w", err)
	}
	if info.IsDir() {
		return Secret{}, fmt.Errorf("sidecar/analyzer: credential path %s is a directory", path)
	}
