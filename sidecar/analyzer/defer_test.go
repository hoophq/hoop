...
func TestDeferForwardsAndAnnotates(t *testing.T) {
	...
	v := ev.EvaluateWith(sqlStmt("DELETE FROM customers", inspect.OpDelete, "customers"), ec)
...
func TestGateRequestOverridesTheTrigger(t *testing.T) {
	...
	v := ev.EvaluateWith(sqlStmt("SELECT * FROM customers", inspect.OpSelect, "customers"), ec)
...
func TestNoGateLeavesTheTriggerInCharge(t *testing.T) {
	...
	ev.EvaluateWith(sqlStmt("SELECT 1", inspect.OpSelect), &policy.EvalContext{})
	if p.calls.Load() != 0 {
		t.Errorf("an unmatched trigger classified anyway: %d calls", p.calls.Load())
	}
	ev.EvaluateWith(sqlStmt("DELETE FROM t", inspect.OpDelete, "t"), &policy.EvalContext{})
	if p.calls.Load() != 1 {
...
func TestStatusNamesWhyTheModelDidNotAnswer(t *testing.T) {
	deleteStmt := sqlStmt("DELETE FROM customers", inspect.OpDelete, "customers")
	...
	t.Run("trigger miss", func(t *testing.T) {
		...
		v := ev.Evaluate(sqlStmt("SELECT 1", inspect.OpSelect))
		if got := v.Annotations[analyzer.MetadataAIStatus]; got != analyzer.StatusSkipped {
			t.Errorf("ai_status = %q, want %q", got, analyzer.StatusSkipped)
		}
	})
		...
		},
		{
			...
			build: func(t *testing.T) (*analyzer.Evaluator, inspect.Statement) {
				return mustNew(t, analyzer.Config{
					...
					Trigger: deleteTrigger(),
					Actions: analyzer.ActionMap{analyzer.RiskHigh: analyzer.ActionDefer},
				}), sqlStmt("SELECT 1", inspect.OpSelect)
			},
...
func TestResponseSideReportsNothing(t *testing.T) {
	...
	resp := sqlStmt("DELETE FROM customers", inspect.OpDelete, "customers")
	resp.Direction = inspect.FromServer
