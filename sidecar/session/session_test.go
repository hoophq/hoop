...
func TestEndIsIdempotent(t *testing.T) {
	s := session.New(inspect.Postgres, session.Identity{Subject: "a"})
	s.End()
	first := s.EndedAt
...
func TestPolicyContextIncludesMetadata(t *testing.T) {
	s := session.New(inspect.HTTP, session.Identity{Subject: "a"})
...
func TestNewSetsFields(t *testing.T) {
	before := time.Now().UTC()
	s := session.New(inspect.HTTP, session.Identity{Subject: "bob"})

	if s.Protocol != inspect.HTTP {
...
func TestGroupsJoinedWithoutTrailingSeparator(t *testing.T) {
	...
	s2 := session.New(inspect.Postgres, session.Identity{
