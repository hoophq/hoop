...
func TestDeferringRuleReportsInsteadOfDenying(t *testing.T) {
	...
	var ec policy.EvalContext
	if v := rules.EvaluateWith(stmt("DELETE FROM customers", inspect.OpDelete, "customers"), &ec); v.Denied {
...
func TestDeferringRuleDoesNotStopALaterDenial(t *testing.T) {
	...
	var ec policy.EvalContext
	v := rules.EvaluateWith(stmt("DELETE FROM customers", inspect.OpDelete, "customers"), &ec)
	if !v.Denied {
...
func TestFirstMatchStillWinsAmongDenyingRules(t *testing.T) {
	...
	var ec policy.EvalContext
	v := rules.EvaluateWith(stmt("DELETE FROM t", inspect.OpDelete, "t"), &ec)
	if v.Rule != "first" {
...
func TestNonMatchingDeferringRuleReportsNothing(t *testing.T) {
	...
	var ec policy.EvalContext
	rules.EvaluateWith(stmt("SELECT 1", inspect.OpSelect), &ec)
...
func TestDeferredDenyWordsNamesTheWordThatFired(t *testing.T) {
	...
	var ec policy.EvalContext
	rules.EvaluateWith(stmt("DELETE FROM customers", inspect.OpDelete, "customers"), &ec)
...
func TestDeferredFindingSurvivesALaterDenial(t *testing.T) {
	rules, err := policy.NewRulesWithScanner([]policy.Rule{
		...
		{Name: "no-select", Type: policy.MatchOperation,
			Operations: []inspect.Operation{inspect.OpSelect}, Message: "no selects"},
...
func TestTableRuleAccessSplit(t *testing.T) {
	write := inspect.Statement{
		Protocol: inspect.Postgres, Direction: inspect.FromClient,
		Text: "DELETE FROM customers", Operation: inspect.OpDelete,
		Tables:    []string{"customers"},
		Relations: []inspect.Relation{{Name: "customers", Access: inspect.AccessWrite}},
	}
	read := inspect.Statement{
		Protocol: inspect.Postgres, Direction: inspect.FromClient,
		Text: "INSERT INTO staging SELECT * FROM customers", Operation: inspect.OpInsert,
		Tables: []string{"staging", "customers"},
		Relations: []inspect.Relation{
			{Name: "staging", Access: inspect.AccessWrite},
			{Name: "customers", Access: inspect.AccessRead},
		},
	}
