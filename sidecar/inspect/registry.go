...
// The registry lets codec packages plug themselves in without sidecar
...
func Register(newCodec func() Codec) {
	if newCodec == nil {
		panic("sidecar: Register called with a nil factory")
	}
	probe := newCodec()
	if probe == nil {
		panic("sidecar: codec factory returned nil")
	}
	p := probe.Protocol()
	...
	if _, dup := registry[p]; dup {
		panic("sidecar: duplicate codec registration for protocol " + string(p))
	}
	registry[p] = newCodec
}
