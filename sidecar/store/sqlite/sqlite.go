...
// # The nested module
//
// The root sidecar module carries libhoop alone and must keep it that way. This
...
func Open(path string) (*Store, error) {
	if path == "" {
		return nil, errors.New("sidecar/store/sqlite: empty path")
	}
...
func open(dataSource string, memory bool) (*Store, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("sidecar/store/sqlite: open: %w", err)
	}
	...
	if memory {
		if s.keepAlive, err = db.Conn(ctx); err != nil {
			db.Close()
			return nil, fmt.Errorf("sidecar/store/sqlite: pin memory connection: %w", err)
		}
	}
...
func (s *Store) Write(ctx context.Context, ev audit.Event) error {
	if ev.SessionID == "" {
		return errors.New("sidecar/store/sqlite: event has no session id")
	}
	...
	if err != nil {
		return fmt.Errorf("sidecar/store/sqlite: begin: %w", err)
	}
	defer tx.Rollback()
	...
	if err := tx.Commit(); err != nil {
		return fmt.Errorf("sidecar/store/sqlite: commit: %w", err)
	}
	return nil
}
...
INSERT INTO events (
	...
	if err != nil {
		return fmt.Errorf("sidecar/store/sqlite: insert event: %w", err)
	}
	return nil
}
...
RETURNING denied_count, error_count`,
	...
	if err != nil {
		return fmt.Errorf("sidecar/store/sqlite: upsert session: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("sidecar/store/sqlite: set verdict: %w", err)
	}
	return nil
}
...
var errBadCursor = errors.New("sidecar/store/sqlite: malformed cursor")
...
func encodeCursor(v any) string {
	...
	if err != nil {
		...
		panic("sidecar/store/sqlite: cursor marshal: " + err.Error())
	}
...
func (s *Store) Sessions(ctx context.Context, f store.SessionFilter) (store.SessionPage, error) {
	...
	if err := s.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM sessions`+where, args...).Scan(&total); err != nil {
		return store.SessionPage{}, fmt.Errorf("sidecar/store/sqlite: count sessions: %w", err)
	}
	...
	if err != nil {
		return store.SessionPage{}, fmt.Errorf("sidecar/store/sqlite: query sessions: %w", err)
	}
	defer rows.Close()
	...
	if err := rows.Err(); err != nil {
		return store.SessionPage{}, fmt.Errorf("sidecar/store/sqlite: scan sessions: %w", err)
	}
	return page, nil
}
...
func scanSession(sc scanner) (store.SessionRecord, int64, error) {
	...
	if err != nil {
		...
		return store.SessionRecord{}, 0, fmt.Errorf("sidecar/store/sqlite: scan session: %w", err)
	}
	rec.ID = session.ID(id)
	rec.Protocol = inspect.Protocol(proto)
...
func (s *Store) Events(ctx context.Context, f store.EventFilter) (store.EventPage, error) {
	...
	if err := s.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM events`+where, args...).Scan(&total); err != nil {
		return store.EventPage{}, fmt.Errorf("sidecar/store/sqlite: count events: %w", err)
	}
	...
	rows, err := s.db.QueryContext(ctx, query, append(pageArgs, f.Limit+1)...)
	if err != nil {
		return store.EventPage{}, fmt.Errorf("sidecar/store/sqlite: query events: %w", err)
	}
	defer rows.Close()
	...
	if err := rows.Err(); err != nil {
		return store.EventPage{}, fmt.Errorf("sidecar/store/sqlite: scan events: %w", err)
	}
	return page, nil
}
...
func scanEvent(sc scanner) (store.EventRecord, error) {
	...
	if err != nil {
		return store.EventRecord{}, fmt.Errorf("sidecar/store/sqlite: scan event: %w", err)
	}
	...
	rec.Protocol = inspect.Protocol(proto)
	rec.Operation = inspect.Operation(op)
	rec.Direction = inspect.Direction(dir)
...
FROM sessions`+where, args...).
	...
	if err != nil {
		return store.Stats{}, fmt.Errorf("sidecar/store/sqlite: stats totals: %w", err)
	}
...
func (s *Store) labelCounts(ctx context.Context, query string, args []any) ([]store.LabelCount, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("sidecar/store/sqlite: breakdown: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&lc.Label, &lc.Count); err != nil {
			return nil, fmt.Errorf("sidecar/store/sqlite: scan breakdown: %w", err)
		}
		out = append(out, lc)
	}
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("sidecar/store/sqlite: breakdown rows: %w", err)
	}
...
func decodeJSON(s string, dst any) error {
	...
	if err := json.Unmarshal([]byte(s), dst); err != nil {
		return fmt.Errorf("sidecar/store/sqlite: decode stored json: %w", err)
	}
	return nil
}
