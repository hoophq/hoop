...
import (
	...
	sqlitestore "github.com/hoophq/hoop/sidecar/store/sqlite"
)
...
func TestSessionLifecycleCounters(t *testing.T) {
	...
	write(t, s, audit.Event{
		...
		Principal: "alice", Protocol: inspect.Postgres, Connection: "appdb",
	...
	for i := range 3 {
		write(t, s, audit.Event{
			...
			Principal: "alice", Operation: inspect.OpSelect, Statement: "SELECT 1", Allowed: true,
		})
	}
	write(t, s, audit.Event{
		Kind: audit.KindViolation, SessionID: "s1", Timestamp: base.Add(5 * time.Second),
		Principal: "alice", Operation: inspect.OpDrop, Statement: "DROP TABLE users",
		Rule: "no-destructive",
	})
	write(t, s, audit.Event{
	...
	if rec.Principal != "alice" || rec.Connection != "appdb" || rec.Protocol != inspect.Postgres {
...
func TestEventWithoutSessionStartCreatesSession(t *testing.T) {
	...
	write(t, s, audit.Event{
		...
		Principal: "bob", Protocol: inspect.HTTP, Connection: "reports",
	...
	if rec.Principal != "bob" || rec.Connection != "reports" || rec.Protocol != inspect.HTTP {
...
func TestEventRoundTrip(t *testing.T) {
	...
	want := audit.Event{
		...
		Principal: "carol", Protocol: inspect.HTTP, Connection: "api",
		Operation: inspect.OpPost, Statement: `{"q":"x"}`,
...
func seedFilterCorpus(t *testing.T, s *sqlitestore.Store) {
	...
	write(t, s, audit.Event{
		...
		Principal: "alice", Protocol: inspect.Postgres, Connection: "appdb",
		Operation: inspect.OpSelect, Statement: "SELECT secret FROM vault", Allowed: true,
	})
	write(t, s, audit.Event{
	...
	write(t, s, audit.Event{
		...
		Principal: "bob", Protocol: inspect.HTTP, Connection: "reports",
		Operation: inspect.OpDrop, Statement: "DROP TABLE ledger", Rule: "no-destructive",
	})
}
...
func TestSessionFilterFieldsNarrow(t *testing.T) {
	...
	cases := []struct {
		...
		{"protocol", store.SessionFilter{Protocol: inspect.Postgres}, []session.ID{"a"}},
...
func TestEventFilterFieldsNarrow(t *testing.T) {
	...
	cases := []struct {
		...
		{"protocol", store.EventFilter{Protocol: inspect.HTTP}, 2},
...
func TestStatsBreakdownUnits(t *testing.T) {
	...
	for i := range 3 {
		write(t, s, audit.Event{Kind: audit.KindStatement, SessionID: "u",
			...
			Principal: "dana", Connection: "appdb", Operation: inspect.OpSelect})
	}
	write(t, s, audit.Event{Kind: audit.KindViolation, SessionID: "u",
		...
		Operation: inspect.OpDrop, Rule: "no-destructive"})
	...
	if st.ByOperation[0].Label != string(inspect.OpSelect) || st.ByOperation[0].Count != 3 {
