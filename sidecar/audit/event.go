...
type Event struct {
	...
	Protocol inspect.Protocol `json:"protocol,omitempty"`

	// Connection is the operator-facing resource name.
	Connection string `json:"connection,omitempty"`

	// Operation is the normalized verb.
	Operation inspect.Operation `json:"operation,omitempty"`
	...
	Direction inspect.Direction `json:"direction,omitempty"`
...
// violations directly.
func StatementEvent(s *session.Session, stmt inspect.Statement, allowed bool, rule, message string) Event {
	kind := KindStatement
	if !allowed {
		kind = KindViolation
	}
...
func statementMetadata(s *session.Session, stmt inspect.Statement) map[string]string {
