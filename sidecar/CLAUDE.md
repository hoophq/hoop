...
- **`test-sidecar` is what proves each nested module builds alone.** An
  earlier version of this file claimed `go test github.com/hoophq/hoop/...`
  did not match `github.com/hoophq/hoop/sidecar`. It does, nested modules
  included, because the module sits under the repository path. So
  `make test-oss` does run these tests. The Makefile comment above the
  `test-sidecar` target says the same thing.
  What that pattern does not do is prove each nested module resolves on its
  own: in workspace mode every module builds against the union of the
  workspace, so a `go.mod` missing a `require` still passes. `test-sidecar`
  enters each module directory, which is the only way to catch that before the
  proxy does. `test-oss` depends on it. Break that dependency and one go.mod
  per optional dependency stops being defended by anything.
