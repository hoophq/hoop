...
ready
  ...
  verify the flow
    sidecar/scripts/dev/pg-kerberos-check.sh

  a password session
    sidecar/scripts/dev/pg-stack.sh psql -c 'SELECT * FROM customers'

  a Kerberos session
    sidecar/scripts/dev/pg-stack.sh kinit -c 'SELECT current_user'
