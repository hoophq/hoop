...
func sqlStmt(text string, op hoopinspect.Operation, tables ...string) hoopinspect.Statement {
...
func deleteTrigger() analyzer.Trigger {
	return analyzer.Trigger{Operations: []hoopinspect.Operation{hoopinspect.OpDelete}}
}
...
func TestHighRiskBlocksAndSurfacesTitle(t *testing.T) {
	...
	v := ev.Evaluate(sqlStmt("DELETE FROM customers", hoopinspect.OpDelete, "customers"))
	if !v.Denied {
...
func TestUnmappedRiskLevelAllows(t *testing.T) {
	...
	if v := ev.Evaluate(sqlStmt("DELETE FROM t", hoopinspect.OpDelete, "t")); v.Denied {
...
func TestTriggerSkipsProvider(t *testing.T) {
	...
	if v := ev.Evaluate(sqlStmt("SELECT 1", hoopinspect.OpSelect)); v.Denied {
...
func TestEmptyTriggerClassifiesNothing(t *testing.T) {
	...
	ev.Evaluate(sqlStmt("DELETE FROM customers", hoopinspect.OpDelete, "customers"))
	if got := p.calls.Load(); got != 0 {
...
func TestCacheCollapsesLiteralVariants(t *testing.T) {
	...
	for _, q := range []string{
		...
		ev.Evaluate(sqlStmt(q, hoopinspect.OpDelete, "customers"))
	}
...
func TestCacheDistinguishesShapes(t *testing.T) {
	...
	ev := mustNew(t, analyzer.Config{
		Provider:  p,
		Trigger:   analyzer.Trigger{Operations: []hoopinspect.Operation{hoopinspect.OpDelete, hoopinspect.OpUpdate}},
		Actions:   analyzer.ActionMap{analyzer.RiskHigh: analyzer.ActionWarn},
		CacheSize: 16,
		CacheTTL:  time.Minute,
	})

	ev.Evaluate(sqlStmt("DELETE FROM customers WHERE id = 1", hoopinspect.OpDelete, "customers"))
	ev.Evaluate(sqlStmt("DELETE FROM payments WHERE id = 1", hoopinspect.OpDelete, "payments"))
	ev.Evaluate(sqlStmt("UPDATE customers SET x = 1", hoopinspect.OpUpdate, "customers"))
...
func TestFailOpenForwardsAndReportsError(t *testing.T) {
	...
	v := ev.Evaluate(sqlStmt("DELETE FROM t", hoopinspect.OpDelete, "t"))
	if v.Denied {
...
func TestBudgetExhaustionAllows(t *testing.T) {
	...
	first := ev.Evaluate(sqlStmt("DELETE FROM a WHERE x = 'p'", hoopinspect.OpDelete, "a"))
	if !first.Denied {
		t.Fatal("the first statement was not classified")
	}
	second := ev.Evaluate(sqlStmt("DELETE FROM b WHERE y = 'q'", hoopinspect.OpDelete, "b"))
	if second.Denied {
...
func TestTimeoutBoundsTheProvider(t *testing.T) {
	...
	start := time.Now()
	ev.Evaluate(sqlStmt("DELETE FROM t", hoopinspect.OpDelete, "t"))
...
func TestRefuseSentinelDeniesWithoutCallingProvider(t *testing.T) {
	...
	v := ev.Evaluate(sqlStmt("DELETE FROM t WHERE cpf = '111'", hoopinspect.OpDelete, "t"))
	if !v.Denied {
...
func TestRedactRewritesTransmittedContent(t *testing.T) {
	...
	ev.Evaluate(sqlStmt("DELETE FROM t WHERE ssn = '123-45-6789'", hoopinspect.OpDelete, "t"))
...
func TestHTTPWithoutBodyIsNotClassified(t *testing.T) {
	...
	stmt := hoopinspect.Statement{
		...
		Direction: hoopinspect.FromClient,
		Operation: hoopinspect.OpPost,
		HTTP:      &hoopinspect.HTTPDetail{Method: "POST", Path: "/anything", Resource: "/anything"},
	}
	if v := ev.Evaluate(stmt); v.Denied {
...
func TestChainShortCircuitsBeforeTheProvider(t *testing.T) {
	...
	v := chain.Evaluate(sqlStmt("DELETE FROM customers", hoopinspect.OpDelete, "customers"))
...
func TestChainPropagatesAnnotationsOnAllow(t *testing.T) {
	...
	v := chain.Evaluate(sqlStmt("DELETE FROM t", hoopinspect.OpDelete, "t"))
	if v.Denied {
...
func TestTwoRulesCanCarryDifferentPrompts(t *testing.T) {
	...
	a.Evaluate(sqlStmt("DELETE FROM x", hoopinspect.OpDelete, "x"))
	if got := p.lastPrompt(); !strings.Contains(got, "ledger rules") {
		t.Errorf("rule A sent %q", got)
	}
	b.Evaluate(sqlStmt("DELETE FROM x", hoopinspect.OpDelete, "x"))
...
func TestPromptChangeInvalidatesCachedVerdicts(t *testing.T) {
	p := &stubProvider{level: analyzer.RiskLow}
	stmt := sqlStmt("DELETE FROM customers WHERE id = 1", hoopinspect.OpDelete, "customers")
