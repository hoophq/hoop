...
var (
	...
	builders  = map[hoopinspect.Protocol]Builder{}
)
...
func Register(name string, build func(Options) (Provider, error)) {
	if name == "" {
		panic("hoopinspect/analyzer: Register called with an empty name")
	}
	if build == nil {
		panic("hoopinspect/analyzer: Register called with a nil builder")
	}
	providerMu.Lock()
	defer providerMu.Unlock()
	if _, dup := providers[name]; dup {
		panic("hoopinspect/analyzer: duplicate provider registration for " + name)
	}
...
func RegisterBuilder(b Builder) {
	if b == nil {
		panic("hoopinspect/analyzer: RegisterBuilder called with nil")
	}
	p := b.Protocol()
	builderMu.Lock()
	defer builderMu.Unlock()
	if _, dup := builders[p]; dup {
		panic("hoopinspect/analyzer: duplicate builder registration for protocol " + string(p))
	}
	builders[p] = b
}

// BuilderFor returns the content builder for a protocol.
func BuilderFor(p hoopinspect.Protocol) (Builder, bool) {
	builderMu.RLock()
