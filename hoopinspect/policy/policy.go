...
type ContextualEvaluator interface {
	...
	EvaluateWith(stmt hoopinspect.Statement, ec *EvalContext) Verdict
}
...
func newRules(rules []Rule, hasScanner bool) (*Rules, error) {
	...
	for i, r := range rules {
		...
		case MatchTable:
			...
			switch r.Access {
			case "", string(hoopinspect.AccessRead), string(hoopinspect.AccessWrite):
			default:
				problems = append(problems, fmt.Sprintf(
...
func (r *Rules) EvaluateWith(stmt hoopinspect.Statement, ec *EvalContext) Verdict {
...
func (r Rule) findingValues(stmt hoopinspect.Statement) map[string]any {
...
func (r Rule) accessMatches(got hoopinspect.Access) bool {
	switch r.Access {
	case "":
		return true
	case string(hoopinspect.AccessWrite):
		return got == hoopinspect.AccessWrite
	case string(hoopinspect.AccessRead):
		return got == hoopinspect.AccessRead
	}
	return true
}
...
func (c Chain) EvaluateWith(stmt hoopinspect.Statement, ec *EvalContext) Verdict {
	var errs error
	for _, e := range c {
		var v Verdict
