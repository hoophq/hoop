...
func TestMaskOnHTTPIsAccepted(t *testing.T) {
	...
	lanes, err := buildLanes(cfg, stubPlugin{entities: []string{"US_SSN"}})
	if err != nil {
		t.Fatalf("buildLanes: %v", err)
	}
...
func TestPIIRuleNamingUndetectedEntityIsRefused(t *testing.T) {
	...
	_, err := buildLanes(cfg, stubPlugin{entities: []string{"US_SSN"}})
	if err == nil {
	...
	if _, err := buildLanes(cfg, stubPlugin{entities: []string{"BR_CPF"}}); err != nil {
