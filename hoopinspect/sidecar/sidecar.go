...
// Main sets it from its argument; a caller embedding Run directly may set it
// instead.
var Version = "dev"
...
func Main(version string, det Plugin) {
	...
	// A "pii" section in a build with no detector is refused rather than
	// ignored. Silently dropping it would leave an operator believing their
	// national-ID masking is on when the binary cannot do it.
