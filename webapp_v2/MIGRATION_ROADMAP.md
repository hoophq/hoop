...
`:create-ai-session-analyzer-rule` (React-rendered since EVL-228 deleted the
CLJS panels; also url-for'd from sidebar constants and
`features/promotion.cljs`). They can only be deleted once `activation_journey`
itself migrates.
...
| ~~B3.2 AI Session Analyzer~~ | ✅ **Done — EVL-148.** `/features/ai-session-analyzer(+rules/new,rules/edit/:ruleName)` in `pages/Features/AiSessionAnalyzer/`: Rules + Configure tabs, provider config, three-tier risk matrix, read-only system prompt panel, free-license gate (1 rule). Activation-journey catalog ported to `pages/Features/AiSessionAnalyzer/templates.js` (`?template=&connections=` deep link, connection **names** not ids). CLJS deleted in EVL-228 (`features/ai_session_analyzer/{main,views/rule_form,views/rule_list,views/configuration_view,views/empty_state}.cljs`, `features/access_request/` whole, the 3 `app.cljs` panels, the `:edit-ai-session-analyzer-rule` route, `:activation-journey/seed-ai-analyzer-template`); `events.cljs` + `subs.cljs` trimmed to the read path the webclient, runbooks runner and activation journey still consume | M | Name is read-only when editing — the API cannot rename a rule. Shadowing `views/rule_form.cljs` strands `features/access_request/{events,subs}.cljs`; both go with the analyzer page files in Track A |
...
| ~~B3.4 Access Request~~ | ✅ **Done — EVL-150.** `/features/access-request(+new,edit/:ruleName)` in `pages/Features/AccessRequest/`, backed by `/access-requests/rules`; the edit route keeps the legacy path-segment shape. Both limits ported: the client-side free-license gate (1 rule) and the gateway's OSS 403, surfaced on save. CLJS deleted in EVL-184 except `features/access_request/events.cljs` + `subs.cljs`, kept for the CLJS AI Session Analyzer rule form; EVL-228 deleted those too | M | Independent |
