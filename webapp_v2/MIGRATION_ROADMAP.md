...
| Clipboard copy/cut blocking | ✅ Done | EVL-143: capture-phase document guard in `hooks/useClipboardGuard` + `utils/clipboardPolicy`, installed in `App.jsx` (App, not Layout — Layout does not wrap `/onboarding/*`). CLJS `events/clipboard.cljs` and its `:clipboard/initialize` dispatch deleted so React is the single owner. The `:gateway->clipboard-disabled?` sub and the webclient CodeMirror keymap stay in CLJS; three ungated CLJS `writeText` call sites are a follow-up |
