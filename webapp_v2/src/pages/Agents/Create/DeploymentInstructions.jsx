...
export function DockerDeployment({ hoopKey }) {
  ...
  return (
    <Stack gap="lg">
      <Stack gap="xs">
        <Text size="sm" fw={700}>Docker image repository</Text>
        <Box
          p="sm"
          style={{ border: '1px solid var(--mantine-color-default-border)', borderRadius: 12 }}
        >
          <Flex gap="sm" align="center">
            <img src="/images/docker-blue.svg" alt="Docker" height={16} />
            <Text size="xs" style={{ flex: 1 }}>{dockerImage}</Text>
      ...
      <Stack gap="xs">
        <Text size="sm" fw={700}>Environment variables</Text>
        <Table variant="vertical" withTableBorder withColumnBorders>
          ...
          <Table.Tbody>
            <Table.Tr>
              ...
              <Table.Td>
                <Flex gap="xs" align="center">
                  <Text size="xs" ff="monospace" style={{ wordBreak: 'break-all' }}>{hoopKey}</Text>
      ...
      <Stack gap="xs">
        <Text size="sm" fw={700}>Manually running in a Docker container</Text>
        <Text size="xs" c="dimmed">If preferred, it is also possible to configure it manually with the following command.</Text>
...
export function KubernetesDeployment({ hoopKey }) {
  ...
  return (
    <Stack gap="lg">
      ...
      <Stack gap="xs">
        <Text size="sm" fw={700}>Standalone deployment</Text>
        <Text size="sm" fw={700}>Helm</Text>
        <Text size="xs" c="dimmed">
          Make sure you have Helm installed.{' '}
          <Anchor href={HELM_DOCS_URL} target="_blank" size="xs">Check the Helm installation guide</Anchor>
        </Text>
        <CodeSnippet code={setVersion} />
        <CodeSnippet code={helmInstall} />
        <Text size="xs" c="dimmed">Using helm manifests</Text>
...
export function LocalDeployment({ hoopKey }) {
  ...
  return (
    <Stack gap="lg">
      ...
      <Stack gap="xs">
        <Text size="sm" fw={700}>Export your HOOP_KEY and run it</Text>
        <Text size="xs" c="dimmed">Run the following command to export your HOOP_KEY and start the agent.</Text>
