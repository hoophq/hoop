  ...
  return (
    <Card padding={0} withBorder>
      <Stack gap={0}>
        ...
        >
          <Text size="xs" fw={600} c="dimmed" tt="uppercase" style={{ width: '40%' }}>
            Runbook parameter
          </Text>
          <Box w={16} />
        ...
        {entries.length === 0 ? (
          <Text size="xs" c="dimmed" p="md">No parameter mapping configured.</Text>
        ) : (
          entries.map(([param, source]) => (
            <Group
              ...
              style={{ borderBottom: '1px solid var(--mantine-color-default-border)' }}
            >
              <Box style={{ width: '40%' }}><Code>{param}</Code></Box>
              <Box w={16} style={{ textAlign: 'center' }}>
                <ArrowRight size={14} color="var(--mantine-color-gray-6)" />
              </Box>
              <Box style={{ flex: 1, minWidth: 0 }}><Code>{source}</Code></Box>
            </Group>
          ))
        )}
      </Stack>
...
function DispatchHistory({ subId }) {
  ...
  if (status === 'loading') {
    return <Text size="xs" c="dimmed" p="md">Loading dispatches…</Text>
  }
  if (status === 'error') {
    return <Text size="xs" c="red" p="md">Failed to load dispatches.</Text>
  }
  if (data.length === 0) {
    return (
      <Stack align="center" gap="xs" py="xl">
        <History size={28} strokeWidth={1.5} color="var(--mantine-color-gray-6)" />
        <Text size="sm" fw={500}>No dispatches yet</Text>
        <Text size="xs" c="dimmed">Matching events will show up here in real time.</Text>
      </Stack>
    )
  }
  return (
    <Stack gap={0}>
      <Group
        ...
        wrap="nowrap"
      >
        <Text size="xs" fw={600} c="dimmed" w={170}>Timestamp</Text>
        <Text size="xs" fw={600} c="dimmed" style={{ flex: 1 }}>Event type</Text>
        <Text size="xs" fw={600} c="dimmed" w={120}>Status</Text>
        <Text size="xs" fw={600} c="dimmed" w={90}>Duration</Text>
      ...
      {data.map((d) => (
        ...
        >
          <Text size="xs" c="dimmed" w={170} ff="monospace" truncate>
            {(d.dispatchedAt || d.createdAt || '').replace('T', ' ').slice(0, 19) || '—'}
          </Text>
          <Box style={{ flex: 1, minWidth: 0 }}><Code>{d.eventType}</Code></Box>
          <Box w={120}><DispatchBadge status={d.status} /></Box>
...
export default function EventRoutingDetail() {
  ...
  if (!sub) {
    return (
      <Stack gap="md">
        ...
        <Text size="sm" c="dimmed">Subscription not found.</Text>
      </Stack>
    )
  }
  ...
  const handleTogglePause = async () => {
    try {
      ...
      notifications.show({
        message: sub.status === 'active' ? 'Subscription paused.' : 'Subscription resumed.',
        color: 'green',
      })
    } catch (e) {
      notifications.show({
    ...
    <Stack gap={0}>
      ...
      {sub.status === 'paused' && (
        <Alert color="yellow" icon={<Pause size={16} />} mb="xl">
          This subscription is paused. Matching events are recorded but not dispatched.
        </Alert>
      )}
      ...
      <Stack gap="xxl">
        <Stack gap="sm">
          <SectionHeader
            ...
            subtitle="Where matching events are dispatched as runbook sessions."
          />
        ...
        <Stack gap="sm">
          <SectionHeader
            ...
            subtitle="The platform event that triggers this subscription. To change it, delete this subscription and create a new one — the parameter mapping depends on the event schema."
          />
          <Group gap="xs">
            {sub.eventTypes?.[0] ? (
              <EventChip name={sub.eventTypes[0]} />
            ) : (
              <Text size="xs" c="dimmed">No event configured.</Text>
            )}
          </Group>
        </Stack>
        ...
        <Stack gap="sm">
          <SectionHeader
            ...
            subtitle="Each runbook parameter is filled from a field on the event payload."
          />
        ...
        <Stack gap="sm">
          <SectionHeader
            ...
            subtitle="Each dispatch produces an audited runbook session. Failures can be replayed."
          />
          <Card padding={0} withBorder>
      ...
      >
        <Stack>
          <Text size="sm">
            ...
            <Text component="span" fw={600}>{sub.name}</Text>
            {`. Existing session history is kept for audit, but the subscription cannot be restored.`}
          </Text>
          <Group justify="flex-end" mt="xs">
            <Button variant="subtle" color="gray" onClick={deleteControls.close}>Cancel</Button>
