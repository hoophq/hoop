...
// The labelled groups, in order. MAIN_ITEMS renders above them without a heading.
const SECTIONS = [
...
export function SidebarExpanded({ navKey }) {
  ...
  const navItemProps = { isAdmin, isApprover, isSelfHosted }
  ...
  const visible = (items) =>
    items.filter((i) => !shouldHide(i, isAdmin, isSelfHosted, isFeatureFlagEnabled, isLicenseFeatureEnabled, isApprover))
        ...
        <Box px="md" className={classes.scrollContent}>
          ...
          {isAdmin && SECTIONS.map(({ id, label, items }) => (
            <Box
              ...
              component="ul"
              role="list"
              aria-labelledby={`sidebar-${id}-heading`}
              mt="xl"
              className={classes.navList}
            >
              <SectionLabel label={label} id={`sidebar-${id}-heading`} />
              <Stack gap="xsAlt" mb="sm">
