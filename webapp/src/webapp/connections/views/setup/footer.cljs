  ...
  (let [sidebar-desktop (rf/subscribe [:sidebar-desktop])]
    (when-not hide-footer?
      [:> Flex {:justify (if (= form-type :update)
                           "start"
                           "center")
                ;; static? => flows in-document at the end of the page (EVL-103);
                ;; default => pinned to the viewport bottom, offset by the sidebar.
                :class (str "bg-white border-t border-[--gray-a6] px-7 py-4 "
                            (if static?
                              "w-full"
                              (str "fixed bottom-0 "
