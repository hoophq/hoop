(ns webapp.components.resource-role-filter
  (:require
...
(defn resource-role-filter
  ...
  (let [open? (r/atom false)
    ...
    (fn [{:keys [selected on-select on-clear]}]
        ...
        [:> Popover.Root {:open @open?
         ...
         [:> Popover.Trigger {:asChild true}
          [:> Button {:size "2"
                      ...
                      :color "gray"}
           [:> Rotate3d {:size 14}]
           (if selected selected "Resource Role")
           (when selected
             [:> X {:size 14
                    :on-click (fn [e]
                                (.stopPropagation e)
                                (on-clear)
                                (close!))}])]]
         [:> Popover.Content {:size "2" :style {:width "300px"}}
          [:> Box
           (when selected
             [:> Box {:class "mb-2 pb-2 border-b border-[--gray-a6]"}
              [:> Flex {:align "center"
                        :gap "2"
           ...
           [:> Box {:mb "2"}
            [:> TextField.Root {:placeholder "Search resource roles"
                                :value @search-term
                                :onChange (fn [e]
