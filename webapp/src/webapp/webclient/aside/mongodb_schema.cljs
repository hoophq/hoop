          ...
          [:div
           [:div {:class "flex items-center gap-small"}
            [:figure {:class "w-3 flex-shrink-0"}
             [:img {:src (str config/webapp-url "/icons/icon-table.svg")}]]
       ...
       (doall
        (for [[db collections] schema]
          ...
          [:div
           [:div {:class "flex items-center gap-small"}
            [:figure {:class "w-3 flex-shrink-0"}
             [:img {:src (str config/webapp-url "/icons/icon-layers-dark-gray.svg")}]]
            [:span {:class (str "hover:text-blue-500 hover:underline cursor-pointer "
             ...
             [:figure {:class "w-4 flex-shrink-0 opacity-30"}
              [:img {:src (if (not= (get @dropdown-status db) :closed)
                            (str config/webapp-url "/icons/icon-cheveron-up.svg")
                            (str config/webapp-url "/icons/icon-cheveron-down.svg"))}]]]]
