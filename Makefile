...
# not a major version.
#
# A replace rather than `go work use`: the consumers pin a released version and
# Go resolves it first, so a checkout ahead of the last tag would be ignored
# and one before the first tag fails outright.
#
...
#   go work edit -dropreplace github.com/hoophq/libhoop
libhoop-dev:
...
# private github.com/hoophq/libhoop module, so a build without credentials
...
# still link the private github.com/hoophq/libhoop module — resolved from
...
# github.com/hoophq/libhoop module and fails outright without credentials
# for the proxy fetch.
test-standalone:
