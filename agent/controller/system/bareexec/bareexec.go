...
func processRequest(client pb.ClientTransport, pkt *pb.Packet) {
	...
	var req pbsystem.BareExecRequest
	...
	log.With("sid", sid).Infof("executing bare exec script, command=%v, scriptsize=%v",
	...
	cmd.Run(func(exitCode int, errMsg string) {
		...
		log.With("sid", sid).Infof("bare exec script finished, exit_code=%v, elapsed=%v, err=%v",
			exitCode, time.Since(startedAt).Round(time.Millisecond), errMsg)

		sendResponse(client, &pbsystem.BareExecResponse{
			SessionID: sid,
			Status:    status,
...
func sendResponse(client pb.ClientTransport, resp *pbsystem.BareExecResponse) {
	payload, pbType, err := resp.Encode()
	if err != nil {
		log.With("sid", resp.SessionID).Warnf("failed encoding bare exec response: %v", err)
		return
	}
	if err := client.Send(&pb.Packet{
		...
		log.With("sid", resp.SessionID).Warnf("failed sending bare exec response: %v", err)
	}
}

func newError(sid, format string, a ...any) *pbsystem.BareExecResponse {
...
func copyBuffer(dst io.Writer, src io.Reader, bufSize int, stream string) chan struct{} {
	...
	go func() {
		...
		log.Infof("[%s] done copying bare exec stream, written=%v, err=%v", stream, wb, err)
		close(doneCh)
	}()
	return doneCh
}
