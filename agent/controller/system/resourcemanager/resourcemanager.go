...
func ProcessRequest(client pb.ClientTransport, pkt *pb.Packet) {
	...
	go processRequestV2(client, pkt)
}

func processRequestV2(client pb.ClientTransport, pkt *pb.Packet) {
	sid := string(pkt.Spec[pb.SpecGatewaySessionID])
	var req pbsystem.BareExecRequest
	if err := json.Unmarshal(pkt.Payload, &req); err != nil {
		sendResponseV2(client, newErrorV2(sid, "unable to decode payload: %v", err))
		return
	}
	...
	cmd.Run(func(exitCode int, errMsg string) {
		...
		sendResponseV2(client, &pbsystem.BareExecResponse{
			SessionID: sid,
			Status:    status,
...
func sendResponseV2(client pb.ClientTransport, resp *pbsystem.BareExecResponse) {
	payload, pbType, err := resp.Encode()
	if err != nil {
...
func newErrorV2(sid, format string, a ...any) *pbsystem.BareExecResponse {
