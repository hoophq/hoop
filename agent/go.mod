...
// Test-only: agent/controller and agent/integration exercise the CLI-side
// proxies (client/proxy) against the agent's own backends. Without this the
// module resolves a PUBLISHED client from the proxy, so those tests would
// compile against whatever was last tagged rather than the working tree —
// which is exactly how a client/agent protocol change ships green and breaks
...
require (
	...
	github.com/hoophq/libhoop v0.0.0-20260911124740-15e4171c522b
...
require (
	...
	github.com/hoophq/alcatraz/ner v0.19.0 // indirect
