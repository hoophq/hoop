...
func toQueryParams(orgID string, opts ...*SessionOption) (string, int) {
	vals := url.Values{}
	limit := defaultLimit
	vals.Set("org_id", fmt.Sprintf("eq.%s", orgID))
	vals.Set("limit", fmt.Sprintf("%v", limit))
	vals.Set("offset", fmt.Sprintf("%v", defaultOffset))
	...
	for _, opt := range opts {
		...
		case OptionLimit:
			...
			if optLimit > defaultLimit {
				vals.Set("limit", fmt.Sprintf("%v", defaultLimit))
				break
			}
			limit = optLimit
...
func (s *Storage) FindAll(ctx *user.Context, opts ...*SessionOption) (*SessionList, error) {
	...
	qs, limit := toQueryParams(ctx.Org.Id, opts...)
	...
	for _, s := range items {
		...
		sessionList.Items = append(sessionList.Items, types.Session{
			ID:               s.ID,
			OrgID:            s.OrgID,
			Script:           types.SessionScript{"data": ""},
			Labels:           s.Labels,
...
func (s *Storage) ListAllSessionsID(fromDate time.Time) ([]*types.Session, error) {
	...
	err := pgrest.New("/sessions?select=id,org_id").List().DecodeInto(&sessionList)
	if err != nil {
		if err == pgrest.ErrNotFound {
			return nil, nil
		}
