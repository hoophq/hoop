...
CREATE UNIQUE INDEX IF NOT EXISTS idx_opa_configs_org_name ON opa_configs(org_id, name);

ALTER TABLE connections ADD COLUMN IF NOT EXISTS opa_config_id UUID NULL
    REFERENCES opa_configs(id) ON DELETE SET NULL;
CREATE INDEX IF NOT EXISTS idx_connections_opa_config_id ON connections(opa_config_id)
    WHERE opa_config_id IS NOT NULL;

ALTER TABLE connections ADD COLUMN IF NOT EXISTS sidecar_id UUID NULL
