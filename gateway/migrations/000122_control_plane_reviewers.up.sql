...
-- Where a sidecar's reviews are posted in Slack. listener_name '' means the
-- whole sidecar; a listener's own row replaces it. With neither, the org's
-- default channel applies, and that channel receives every review anyway, as
-- it does on the gateway. listener_name is not a foreign key: a listener is an
-- element of sidecars.configuration, and a configuration write drops the rows
-- of listeners it removed. An empty list is no row, not a row that means none.
