...
-- (ADR-0019). The control plane pulls Slack user groups, the identity provider
-- pushes users and groups over SCIM, or an admin imports a file. All of them
-- land in users and user_groups, which is what every approval reads; these
-- tables only record where a row came from. No vendor secret is stored.
...
-- scim or file. external_id is the source's id, which is how the next run
-- finds the same person again after an email change.
...
-- SCIM user names are unique per source ignoring case, as RFC 7643 asks.
CREATE UNIQUE INDEX IF NOT EXISTS idx_directory_users_user_name
    ON directory_users (org_id, source, lower(user_name));
...
-- as user_groups.name. The id is what SCIM addresses a group by, so a rename
-- keeps the same row.
