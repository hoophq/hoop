...
func (s *Service) FindById(context *user.Context, id string) (*Review, error) {
...
func (s *Service) Persist(context *user.Context, review *Review) error {
	...
	for i, r := range review.ReviewGroups {
		...
		reviewGroupIds = append(reviewGroupIds, review.ReviewGroups[i].Id)
	}
