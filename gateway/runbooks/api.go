...
func (h *Handler) RunExec(c *gin.Context) {
	...
	case resp := <-clientResp:
		if resp.exitCode != nilExitCode {
			responseJSON["exit_code"] = resp.exitCode
		}
		if resp.err != nil {
			responseJSON["message"] = resp.err.Error()
...
func (h *Handler) getConnection(ctx *user.Context, c *gin.Context, connectionName string) (string, error) {
	...
	if err != nil {
		...
		c.JSON(http.StatusInternalServerError, gin.H{
			"session_id": nil,
			"message":    "failed retrieving connection"})
		return "", err
	}
	if conn == nil {
		c.JSON(http.StatusNotFound, gin.H{
...
func (h *Handler) getRunbookConfig(ctx *user.Context, c *gin.Context, connectionName string) (*templates.RunbookConfig, error) {
	...
	if err != nil {
		...
		c.JSON(http.StatusInternalServerError, gin.H{
			"session_id": nil,
			"message":    "failed retrieving runbook plugin"})
	...
	if !hasConnection {
		c.JSON(http.StatusUnprocessableEntity, gin.H{
			"session_id": nil,
			"message":    "plugin is not enabled for this connection"})
