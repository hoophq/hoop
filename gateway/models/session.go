...
// SessionAnalyticsSnapshot holds a capture of everything analytics cares about for a session.
type SessionAnalyticsSnapshot struct {
	...
	// Guardrails
	GuardrailsActivated bool `gorm:"column:guardrails_activated"`

	// Access request rules
	AccessRequestActivated      bool           `gorm:"column:access_request_activated"`
	AccessRequestForceApprovals pq.StringArray `gorm:"column:force_approval_groups;type:text[]"`
	AccessRequestMinApprovals   *int           `gorm:"column:min_approvals"`
}

// GetSessionPropertiesByID fetches all the fields needed to build session analytics properties
// in a single query, joining sessions → connections → agents, guardrail_rules_connections, and access_request_rules.
func GetSessionPropertiesByID(orgID, sessionID string) (*SessionAnalyticsSnapshot, error) {
	...
	SELECT
		...
		COALESCE(a.metadata->>'version', '') AS agent_version,
		EXISTS (
			SELECT 1
			FROM private.guardrail_rules_connections grc
			WHERE grc.connection_id = c.id AND grc.org_id = c.org_id
		) AS guardrails_activated,
		CASE WHEN arr.id IS NOT NULL THEN true ELSE false END AS access_request_activated,
		arr.force_approval_groups,
		arr.min_approvals
	FROM private.sessions s
	LEFT JOIN private.connections c
		ON c.org_id = s.org_id AND c.name = s.connection
	LEFT JOIN private.agents a
		ON a.id = c.agent_id AND a.org_id = c.org_id
	LEFT JOIN private.access_request_rules arr
		ON arr.org_id = s.org_id AND arr.connection_names @> ARRAY[s.connection]
	WHERE s.org_id = ? AND s.id = ?
	LIMIT 1
	`, orgID, sessionID).First(result).Error
	if err != nil {
		return nil, err
	}
	return result, nil
