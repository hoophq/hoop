...
func (s *Storage) Persist(ctx *user.Context, c *Connection) (int64, error) {
...
func (s *Storage) Evict(ctx *user.Context, connectionName string) error {
	return pgrest.New("/connections?org_id=eq.%v&name=eq.%v", ctx.Org.Id, connectionName).
		Delete().
		Error()
...
func (s *Storage) FindAll(ctx *user.Context) ([]BaseConnection, error) {
	var items []pgrest.Connection
	err := pgrest.New("/connections?select=*,orgs(id,name)&org_id=eq.%v", ctx.Org.Id).
		List().
		DecodeInto(&items)
...
func (s *Storage) FindOne(ctx *user.Context, name string) (*Connection, error) {
	var c pgrest.Connection
	err := pgrest.New("/connections?select=*,orgs(id,name)&org_id=eq.%v&name=eq.%v", ctx.Org.Id, name).
		FetchOne().
		DecodeInto(&c)
	if err != nil {
	...
	return &Connection{
		BaseConnection: BaseConnection{
			c.ID, c.Name, "", c.Command, Type(c.Type),
			c.AgentID, DBSecretProvider},
		Secret: secrets,
	}, nil
