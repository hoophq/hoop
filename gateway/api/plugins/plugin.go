...
func CreatePluginConnection(c *gin.Context) {
	...
	err := models.CreatePluginConnection(&models.PluginConnection{
		ID:           resourceID,
		OrgID:        ctx.OrgID,
	...
	c.JSON(http.StatusCreated, openapi.PluginConnection{
		ID:           resourceID,
		PluginID:     req.PluginID,
		ConnectionID: req.ConnectionID,
		Config:       req.Config,
...
func UpdatePluginConnection(c *gin.Context) {
	...
	err := models.UpdatePluginConnection(&models.PluginConnection{
		ID:           c.Param("id"),
		OrgID:        ctx.OrgID,
		PluginID:     req.PluginID,
		ConnectionID: req.ConnectionID,
	...
	case nil:
		c.JSON(http.StatusOK, openapi.PluginConnection{
			ID:           c.Param("id"),
			PluginID:     req.PluginID,
			ConnectionID: req.ConnectionID,
			Config:       req.Config,
...
func GetPluginConnection(c *gin.Context) {
	ctx := storagev2.ParseContext(c)
	pluginConn, err := models.GetPluginConnection(ctx.OrgID, c.Param("id"))
	switch err {
	case models.ErrNotFound:
		c.JSON(http.StatusNotFound, gin.H{"message": err.Error()})
		return
	case nil:
		c.JSON(http.StatusOK, openapi.PluginConnection{
			ID:           pluginConn.ID,
			PluginID:     pluginConn.PluginID,
			ConnectionID: pluginConn.ConnectionID,
			Config:       pluginConn.Config,
		})
	default:
