...
// connectionName builds a globally unique connection name scoped to a resource.
// The DB enforces a unique constraint on (org_id, name), so the same role
// (e.g. "analytics_readonly") on different resources must produce different names.
func connectionName(resourceName, role string) string {
...
func PlanResource(c *gin.Context) {
	...
	log.Infof("plan-resource: starting plan for resource=%s org=%s", name, ctx.OrgID)

	resource, err := models.GetResourceByName(models.DB, ctx.OrgID, name, ctx.IsAdmin())
	if err != nil && !errors.Is(err, gorm.ErrRecordNotFound) {
		log.Errorf("plan-resource: failed to get resource=%s: %v", name, err)
		httputils.AbortWithErr(c, http.StatusInternalServerError, err, "failed retrieving resource: %v", err)
		return
	}
	if resource == nil {
		log.Warnf("plan-resource: resource=%s not found", name)
		c.JSON(http.StatusNotFound, gin.H{"message": "resource not found"})
		return
	}
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		log.Warnf("plan-resource: bad request for resource=%s: %v", name, err)
		c.JSON(http.StatusBadRequest, gin.H{"message": err.Error()})
		return
	}

	// Check if a connection with this role already exists for this resource.
	// Connection names are globally unique per org, so we prefix with the resource name.
	connName := connectionName(name, req.Role)
	existingConn, _ := models.GetResourceConnectionByName(models.DB, ctx.OrgID, name, connName)
	status := "Create"
	if existingConn != nil {
		status = "Update"
	}
	...
	log.Infof("plan-resource: resource=%s role=%s status=%s plan_id=%s", name, req.Role, status, planID)
...
// BatchPlanResources
//
//	@Summary		Plans role provisioning for multiple resources in a single call
//	@Description	Accepts a batch of items, checks each role, and returns plan results.
//	@Description	No data is persisted — plans are held in memory for subsequent apply.
//	@Tags			Resources
//	@Accept			json
//	@Produce		json
//	@Param			request	body		openapi.BatchPlanRequest		true	"The batch plan request"
//	@Success		200		{object}	openapi.BatchPlanResponse
//	@Failure		400		{object}	openapi.HTTPError
//	@Router			/resources/plan/batch [post]
func BatchPlanResources(c *gin.Context) {
	ctx := storagev2.ParseContext(c)

	var req openapi.BatchPlanRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		log.Warnf("batch-plan: bad request: %v", err)
		c.JSON(http.StatusBadRequest, gin.H{"message": err.Error()})
		return
	}

	log.Infof("batch-plan: received %d items, org=%s", len(req.Items), ctx.OrgID)
	var results []openapi.BatchPlanResultItem
	var failedCount int

	for _, item := range req.Items {
		resource, err := models.GetResourceByName(models.DB, ctx.OrgID, item.ResourceName, ctx.IsAdmin())
		if err != nil || resource == nil {
			log.Warnf("batch-plan: resource=%s not found (err=%v)", item.ResourceName, err)
			failedCount++
			results = append(results, openapi.BatchPlanResultItem{
		...
		connName := connectionName(item.ResourceName, item.Role)
		existingConn, _ := models.GetResourceConnectionByName(models.DB, ctx.OrgID, item.ResourceName, connName)
		status := "Create"
		if existingConn != nil {
			status = "Update"
		}
	...
	log.Infof("batch-plan: completed %d items, %d failed", len(results), failedCount)
	c.JSON(http.StatusOK, openapi.BatchPlanResponse{Results: results})
}

// BatchApplyPlans
//
//	@Summary		Applies multiple plans in a single call
//	@Description	Looks up each plan from the in-memory store, creates connections in the DB.
//	@Tags			Resources
//	@Accept			json
//	@Produce		json
//	@Param			request	body		openapi.BatchApplyRequest		true	"The batch apply request"
//	@Success		200		{object}	openapi.BatchApplyResponse
//	@Failure		400		{object}	openapi.HTTPError
//	@Router			/resources/apply/batch [post]
func BatchApplyPlans(c *gin.Context) {
	ctx := storagev2.ParseContext(c)

	var req openapi.BatchApplyRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		log.Warnf("batch-apply: bad request: %v", err)
		c.JSON(http.StatusBadRequest, gin.H{"message": err.Error()})
		return
	}

	log.Infof("batch-apply: received %d plan IDs, org=%s", len(req.PlanIDs), ctx.OrgID)
	var results []openapi.BatchApplyResultItem

	for _, planID := range req.PlanIDs {
		val, ok := planStore.Load(planID)
		if !ok {
			log.Warnf("batch-apply: plan_id=%s not found or already applied", planID)
			results = append(results, openapi.BatchApplyResultItem{
				PlanID: planID,
				Status: "ApplyFailed",
				Error:  "plan not found or already applied",
			})
			continue
		}
		plan := val.(*storedPlan)
		if plan.OrgID != ctx.OrgID {
			log.Warnf("batch-apply: plan_id=%s org mismatch (plan=%s, ctx=%s)", planID, plan.OrgID, ctx.OrgID)
			results = append(results, openapi.BatchApplyResultItem{
				PlanID: planID,
				Status: "ApplyFailed",
				Error:  "plan not found for this organization",
			})
			continue
		}

		resource, err := models.GetResourceByName(models.DB, ctx.OrgID, plan.ResourceName, true)
		if err != nil || resource == nil {
			log.Warnf("batch-apply: plan_id=%s resource=%s not found (err=%v)", planID, plan.ResourceName, err)
			results = append(results, openapi.BatchApplyResultItem{
				PlanID:   planID,
				Status:   "ApplyFailed",
				RoleName: plan.Role,
		...
		connName := connectionName(plan.ResourceName, plan.Role)
		...
		err = models.UpsertResourceConnection(models.DB, conn)
		if err != nil {
			log.Errorf("batch-apply: failed to upsert connection %s for resource %s: %v", connName, plan.ResourceName, err)
			results = append(results, openapi.BatchApplyResultItem{
				PlanID:    planID,
				Status:    "ApplyFailed",
				SessionID: sessionID,
		...
		log.Infof("batch-apply: applied plan_id=%s role=%s resource=%s conn_id=%s", planID, plan.Role, plan.ResourceName, conn.ID)
		results = append(results, openapi.BatchApplyResultItem{
			PlanID:    planID,
			Status:    "Applied",
			SessionID: sessionID,
	...
	var appliedCount, failedApplyCount int
	...
	log.Infof("batch-apply: completed %d items, %d applied, %d failed", len(results), appliedCount, failedApplyCount)
	c.JSON(http.StatusOK, openapi.BatchApplyResponse{Results: results})
}
...
func ApplyPlan(c *gin.Context) {
	...
	log.Infof("apply-plan: starting apply for resource=%s org=%s", name, ctx.OrgID)

	var req openapi.ResourceApplyRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		log.Warnf("apply-plan: bad request for resource=%s: %v", name, err)
		c.JSON(http.StatusBadRequest, gin.H{"message": err.Error()})
		return
	}
	...
	if !ok {
		log.Warnf("apply-plan: plan_id=%s not found or already applied", req.PlanID)
	...
	if plan.ResourceName != name || plan.OrgID != ctx.OrgID {
		log.Warnf("apply-plan: plan_id=%s mismatch (plan_resource=%s, request_resource=%s)", req.PlanID, plan.ResourceName, name)
	...
	if err != nil {
		log.Errorf("apply-plan: failed to get resource=%s: %v", name, err)
		httputils.AbortWithErr(c, http.StatusInternalServerError, err, "failed retrieving resource: %v", err)
		return
	}
	if resource == nil {
		log.Warnf("apply-plan: resource=%s not found", name)
		c.JSON(http.StatusNotFound, gin.H{"message": "resource not found"})
		return
	}
	...
	connName := connectionName(name, plan.Role)
	conn := &models.Connection{
		OrgID:              ctx.OrgID,
		Name:               connName,
		ResourceName:       name,
	...
	err = models.UpsertResourceConnection(models.DB, conn)
	if err != nil {
		log.Errorf("apply-plan: failed to upsert connection %s for resource %s: %v", connName, name, err)
	...
	log.Infof("apply-plan: applied plan_id=%s role=%s resource=%s conn_id=%s", req.PlanID, plan.Role, name, conn.ID)
